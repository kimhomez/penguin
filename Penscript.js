function init() {
  ...
  const animatePenguin = (penguin) => {
    ...
    setMargin(
      penguinSprite,
      `-${animationFrames[animation][i] * cellSize}`,
      `-${cellSize * directions[sprites[penguinData.direction]]}`
    );
    penguinData.frame = i === animationFrames[animation].length - 1 ? 0 : i + 1;
    penguinData.frameTimer = setTimeout(
  ...
  const randomDirection = () => {
    return turnDirections.filter((t) => t !== "turn")[
      Math.floor(Math.random() * turnDirections.length - 1)
    ];
  };
  ...
  const clickedAngle = () => {
    const angle =
      radToDeg(
        Math.atan2(penguinData.pos.y - control.y, penguinData.pos.x - control.x)
      ) - 90;
  ...
  const createMark = (penguin, angle) => {
    ...
    mark.style.transform = `rotate(${angle}deg) scale(${
      penguinData.stepLeft ? -1 : 1
    }, 1)`;
  ...
  const createJumpMark = (penguin) => {
    const {
      left,
      top
    } = penguin.childNodes[1].childNodes[3].childNodes[1].getBoundingClientRect();
    const mark = document.createElement("div");
  ...
  const touchedTheStar = (x, y) => {
    return (
      ...
      (overlap(control.x, penguinData.pos.x) &&
        overlap(control.y, penguinData.pos.y))
    );
  };
  ...
  const moveAbout = (penguin) => {
    ...
    if (touchedTheStar(x, y)) {
      checkBoundaryAndUpdatePenguinPos(
        control.x,
        control.y,
        penguin,
        penguinData
  ...
  const svgWrapper = ({ content, w, h, frameWidth, spriteNo }) =>
    `<svg x="0px" y="0px" width="${frameWidth}%" height="${
      100 / spriteNo
